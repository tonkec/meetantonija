...
export default function CV(props){
    ...
    const onSearch = (e) => {
        const filteredProject = projects.filter((project) => {
            if (e.target.value.trim() !== "") {
                const nameOfTheClient = project.title.toLowerCase().startsWith(e.target.value.toLowerCase())
    ...
    return <>
        ...
        <section className="bg-gray">
            <input onChange={onSearch} className="bg-transparent text-white placeholder-white w-full py-10 px-20 text-4xl border-none focus:outline-none" type="search" placeholder="Search by the name of a client" />
        </section>
