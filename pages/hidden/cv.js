  ...
  const onSearch = (e) => {
    const filteredProject = projects.filter((project) => {
      if (e.target.value.trim() !== "") {
        const nameOfTheClient =
          project.title
            .toLowerCase()
            .startsWith(e.target.value.toLowerCase()) ||
          project.hashtags.filter((hastag) =>
            hastag.toLowerCase().startsWith(e.target.value.toLowerCase())
          )[0];
          ...
          filteredProjects.map((project) => {
            return (
              <div key={project.title} className="project">
                <div>
                  <h4 className={`${heading4} underline mb-5 mt-20`}>
                ...
                <div className="md:pl-10 pl-4 mt-6 md:mt-10 mb-5">
                  <ul className="list-disc">
                    {project.tasks.map((task, i) => (
                      <li key={i} className="text-md md:text-2xl">
                        {task}
                      </li>
                    ))}
                  </ul>
                ...
                <div className="flex hashtags">
                  {project.hashtags.map((hash, i) => (
                    <a key={i} href={`/tags/${hash}`}>
                      <span className="font-bold inline-block mb-10 mx-2">
                        #{hash}
                      </span>
                    </a>
                  ))}
