...
function MyApp({ Component, pageProps, router }) {
  ...
  const onSearch = (e) => {
    const filteredProject = projects.filter((project) => {
      if (e.target.value.trim() !== "") {
        const nameOfTheClient = project.title
          .toLowerCase()
  ...
  return (
    <>
      ...
      <Layout>
        {isSearchShown && (
          <section className="bg-ternary fixed inset-0 z-max">
            <input
              autoFocus
              onChange={onSearch}
              className="bg-white text-black placeholder-black w-full py-4 md:py-10 px-10 text-2xl lg:text-4xl focus:outline-none"
              type="search"
              placeholder="Search by the name of a client"
            />
              ...
              filteredProjects.map((project) => {
                return (
                  <a
                    ...
                    key={project.title}
                    className="px-10 block"
                  >
                    <h4 className={`${heading4} underline mb-5 mt-5`}>
                      {project.title}
                    </h4>
                  </a>
                );
